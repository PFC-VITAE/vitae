...
class ObjectStore(IObjectStore):
    ...
    def insert_dl_resume(self, cpf, resume, file_extension):
        if file_extension == "xml":
            ...
            file_content = json.dumps(
                [curso.__dict__ for curso in resume], ensure_ascii=False
            )
    ...
    def list_dl_objects(self, prefix):
        try:
            objects = self.__dl_client.list_objects(
    ...
    def get_dl_object(self, object_name):
        try:
            response = self.__dl_client.get_object(minio_bucket, object_name)
