...
class ConsolidateCandidateData:
    ...
    def __init__(
        self,
        vitae_extractor: IVitaeExtractor,
        candidate_repository: ICandidateRepository,
        object_storage: IObjectStore,
    ):
    ...
    def extract_resume(self, object):
        ...
        id = self.__vitae_extractor.get_ID(
            cpf=object.cpf, full_name=object.full_name, birth_date=object.birth_date
        )
    ...
    def execute(self):
        try:
            ...
            if (
                len(candidates_data) < 1
                or candidates_data[0].updated_last < datetime.now().year
            ):
                ...
                for candidate in candidates_list:
                    ...
                    self.__object_storage.insert_dl_resume(
                        cpf=candidate.cpf, resume=resume, file_extension="xml"
                    )
                    self.__object_storage.insert_dl_resume(
                        cpf=candidate.cpf,
                        resume=candidate.dgp_courses,
                        file_extension="json",
                    )
                ...
                return "All candidates are up to date"
